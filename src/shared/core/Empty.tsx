...
const Empty = ({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        <h2 className="pb-1 text-center text-base font-semibold leading-relaxed text-gray-700 dark:text-gray-200">
          {title}
        </h2>
        <p className="pb-4 text-center text-sm font-normal leading-snug text-gray-700 dark:text-gray-200">
          {subTitle}
        </p>
        {link && link?.length > 0 && (
          <div
          ...
          >
            <Link
              href={link ? link : ""}
              className="w-fit rounded-full bg-tertiary-700 px-6 py-2.5 text-center text-xs font-semibold leading-4 text-white transition-all duration-500 hover:bg-tertiary-800"
            >
              {pathName}
            </Link>
          </div>
