...
import { UpdateScenarioSchema } from "../../schema/risk.schema";
import { Risk } from "./RiskScenario";
...
const UpdateRiskScenario: React.FC<RiskScenarioFormProps> = ({
  isOpen,
  onClose,
  mutate,
  data
  ...
  // Initialize showCustomRiskType based on whether the risk type is in predefined options
  const [showCustomRiskType, setShowCustomRiskType] = useState(false);

  const initialValue = useMemo(() => {
    const isCustomRiskType =
      data?.risk_type && !RiskTypeOptions.includes(data.risk_type);

    return {
      riskId: data?.id || "",
      riskName: data?.name || "",
      description: data?.description || "",
      probability: data?.likelihood
        ? (Object.keys(LikelihoodLabels) as unknown as number[]).find(
            (key) =>
              LikelihoodLabels[key as keyof typeof LikelihoodLabels] ===
              data.likelihood
          ) || 1
        : 1,
      impact: data?.impact
        ? (Object.keys(ImpactLabels) as unknown as number[]).find(
            (key) =>
              ImpactLabels[key as keyof typeof ImpactLabels] === data.impact
          ) || 1
        : 1,
      riskLevel: data?.risk_level || "",
      riskType: isCustomRiskType ? "Other" : data?.risk_type || "",
      customRiskType: isCustomRiskType ? data?.risk_type : "",
      mitigationStrategy: data?.mitigation_strategy || "",
      tags: data?.tags || [],
      controlEffectiveness: data?.control_effectiveness || 0,
      notes: data?.notes || ""
    };
  }, [
    data?.id,
    ...
    data?.notes // Fixed typo: was data?.noteș
  ]);

  const formik = useFormik<FormValues>({
    initialValues: initialValue,
    validationSchema: UpdateScenarioSchema,
    ...
    onSubmit: async (values, { resetForm }: FormikHelpers<FormValues>) => {
      try {
        ...
        const res = await mutation(`risk?doc_id=${data?.doc_id}`, {
          method: "PUT",
          isAlert: false,
          body: {
            id: values.riskId,
  ...
  useEffect(() => {
    if (data && !RiskTypeOptions.includes(data?.risk_type)) {
      setShowCustomRiskType(true);
      formik.setFieldValue("customRiskType", data?.risk_type || "");
    } else {
      setShowCustomRiskType(false);
    }
  }, [data]);
...
export default UpdateRiskScenario;
