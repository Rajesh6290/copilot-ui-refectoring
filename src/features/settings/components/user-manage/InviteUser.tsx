...
import { InviteUserValidationSchema } from "../../schema/user.schema";
      ...
      try {
        const res = await mutation("user/invite", {
          ...
          body: {
            email: values.email,
            role_id: values.role,
            groups: values.group?.map((item: GroupData) => item?.group_id)
          }
        });
        if (res?.status === 200) {
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {/* <div className="mt-2 text-sm text-blue-600 hover:underline">
              <a href="#" onClick={() => router.push(`/settings/roles`)}>
                Create custom role
              </a>
            </div> */}
          </div>
                  ...
                  {groups?.map((item) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleOption(item, formik.setFieldValue);
                        }}
