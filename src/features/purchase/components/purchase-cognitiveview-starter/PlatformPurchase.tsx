...
import CheckoutForm, { Feature } from "./CheckoutForm";
...
const PlatformPurchase = () => {
  ...
  const defaultRedirectUrl =
    "/purchase/purchase-cognitiveview-starter?featureId=standard_starter_plan_team&frequency=month";
  ...
  useEffect(() => {
    document.title =
      data && data?.name?.length > 0
        ? `Purchase ${data?.name} pack | Cognitiveview`
        : "Purchase | Cognitiveview";
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : "Error fetching plan"
        );
        window.location.href = defaultRedirectUrl; // Use direct navigation instead of router
      }
    }
      ...
      try {
        ...
        const body = {
          customer_data: {
            ...
            source: type && type.length > 0 ? type : undefined,
            temporary_client_id:
        ...
        const res = await mutation(
          `checkout?subscription_id=${featureId}&payment_freq=${frequency}`,
          {
            method: "POST",
            isAlert: false,
            body
        ...
        if (res?.status === 200 && res?.results?.payment_link) {
          toast.success("Submission successful!");
          resetForm();
          window.location.href = res.results.payment_link;
        }
      } catch (error) {
        toast.error(
...
export default PlatformPurchase;
