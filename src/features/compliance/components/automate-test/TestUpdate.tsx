...
import { UpdateTestValidationSchema } from "../../schema/test.schema";
import { Test } from "./TestRegister";
...
const TestUpdate: React.FC<TestUpdateProps> = ({
  open,
  onClose,
  mutate,
  updateData
  ...
  //   const { data: controlData, isValidating: controlLoading } = useSwr(
  //     applicationId?.length > 0
  //       ? `control/filter?application_id=${applicationId}`
  //       : null,
  //   );

  // Sync applicationId when dialog opens with updateData
  //   useEffect(() => {
  //     if (open) {
  //       setApplicationId(updateData?.application_id || "");
  //     } else {
  //       setApplicationId("");
  //     }
  //   }, [open, updateData?.application_id]);
  ...
  // Convert ISO UTC to local time (HH:mm format)
  const convertISOToLocalTime = (isoString: string): string => {
    if (!isoString) {
      return "";
    }
    try {
  ...
  const initialValues: TestFormValues = {
    ...
    frequency: updateData?.frequency
      ? FREQUENCY_TO_SECONDS[
          updateData.frequency as keyof typeof FREQUENCY_TO_SECONDS
        ] || ""
      : "",
    preferredRunTime: updateData?.preferred_run_time
      ? convertISOToLocalTime(updateData.preferred_run_time)
      : "",
    application: updateData?.application_id || "",
    notes: updateData?.notes || ""
  };

  const handleSubmit = async (values: TestFormValues): Promise<void> => {
    try {
      const res = await mutation(`test?test_id=${updateData?.test_id}`, {
        method: "PUT",
        isAlert: false,
        body: {
  ...
  return (
    ...
    >
      ...
      <Formik
        ...
        validationSchema={UpdateTestValidationSchema}
          ...
          <Form className="w-full">
            ...
            <div className="max-h-[70vh] overflow-y-auto p-6">
              <div className="space-y-6">
                ...
                {/* Application Field (Single Select) */}
                <div className="space-y-2">
                  <span className="block text-sm font-medium text-gray-700 dark:text-gray-200">
                  ...
                  {touched.application && errors.application && (
                    <p className="text-sm text-red-500">{errors.application}</p>
                  )}
                </div>
