...
export interface TestControl extends Record<string, unknown> {
  id: string;
  doc_id: string;
  ...
  enforcement_type: "Manual" | "Automated";
...
const StatusBadge = ({
  ...
  <div className="flex flex-col items-center justify-center gap-1">
    <span
      className={`inline-flex w-fit items-center justify-center gap-2 rounded-lg border px-3 py-1.5 text-sm font-medium capitalize ${getStatusStyles(status)}`}
    >
...
const TestControl = ({ testId }: { testId: string }) => {
  const router = useRouter();
  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(10);
  const { data, isValidating } = useSwr(
    `tests/control?page=${page + 1}&limit=${pageSize}&test_id=${testId}`
  );
  return (
    <CustomTable<TestControl>
      columns={[
        {
          field: "control_id",
          title: "Control ID",
      ...
      localization={{
        ...
        body: {
          emptyDataSourceMessage:
            'You have not added any controls to this test. You can click on the "Add Control" button to add controls to this test.'
        }
      }}
      customToolbar={<></>}
      options={{
