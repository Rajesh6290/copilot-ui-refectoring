...
import { UpdateFindingValidationSchema } from "../../schema/finding.schema";
const UpdateFinding = ({
  open,
  onClose,
  mutate,
  ...
  const initialValues = {
    title: findingData?.heading || "",
    message: findingData?.message || "",
    status: findingData?.status || "other",
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      const res = await mutation(`finding?finding_id=${findingData.id}`, {
        method: "PUT",
        body: payload,
        isAlert: false
      });
      ...
      if (res?.status === 200) {
        onClose();
        mutate();
        toast.success(res?.results?.message || "Finding updated successfully");
      }
    } catch (error) {
      toast.error(
  ...
  return (
    ...
    >
      <Formik
        ...
        validationSchema={UpdateFindingValidationSchema}
